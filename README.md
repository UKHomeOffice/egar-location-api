# API for Storing information on location for use in eGAR 
...
By default the service runs by default on port 8081. But to change this, add to the configuration the following:
```
--server.port=8080
```
...
The system will connect to a db instance on the same host with the database `dev_location_api`.
...
```
mvn -Dflyway.url=jdbc:postgresql://localhost/dev_location_api -Dflyway.user=postgres -Dflyway.password=mysecretpassword flyway:migrate 
```
